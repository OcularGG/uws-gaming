...
export default function CreatePortBattlePage() {
  ...
  const updateRole = (setupIndex: number, roleIndex: number, field: keyof FleetRole, value: any) => {
    ...
    const currentRole = newSetups[setupIndex].roles[roleIndex]
    newSetups[setupIndex].roles[roleIndex] = { ...currentRole, [field]: value }

    // Auto-update BR value when ship name changes
    if (field === 'shipName') {
  ...
  const updateRolePerks = (setupIndex: number, roleIndex: number, perks: PerkSelection[]) => {
    updateRole(setupIndex, roleIndex, 'perks', perks)
  }
  ...
  const duplicateRole = (setupIndex: number, roleIndex: number) => {
    ...
    roleToDuplicate.perks = [...roleToDuplicate.perks] // Deep copy perks array
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setFormData(prev => ({ ...prev, brLimit: parseInt(e.target.value) || 2500 }))}
                className="w-full border-2 border-navy-dark rounded px-3 py-2 focus:outline-none focus:border-brass transition-colors"
                style={{fontFamily: 'Crimson Text, serif'}}
                required
              />
                ...
                {setup.roles.map((role, roleIndex) => {
                  ...
                  return (
                    <div key={roleIndex} className="border border-navy-dark/30 rounded-lg p-3 bg-sail-white/50">
                      ...
                      {/* Collapsible Captain Perks Section */}
                      <div className="mt-4 border-t border-navy-dark/20 pt-4">
                        <button
                          type="button"
                          onClick={() => togglePerks(setupIndex, roleIndex)}
                          className="w-full text-left text-sm font-medium text-navy-dark flex justify-between items-center hover:bg-navy-dark/5 p-2 rounded transition-colors"
                          style={{fontFamily: 'Cinzel, serif'}}
                        >
                          Captain Perks
                      ...
                      {/* Collapsible Load Calculator Section */}
                      <div className="mt-4 border-t border-navy-dark/20 pt-4">
                        <button
                          type="button"
                          onClick={() => toggleLoadCalculator(setupIndex, roleIndex)}
                          className="w-full text-left text-sm font-medium text-navy-dark flex justify-between items-center hover:bg-navy-dark/5 p-2 rounded transition-colors"
                          style={{fontFamily: 'Cinzel, serif'}}
                        >
                          Load Calculator
                          <span className="text-xs">
                            {expandedLoadCalculators[loadKey] ? '▼' : '▶'}
                          </span>
                        </button>
                        {expandedLoadCalculators[loadKey] && (
                          <div className="mt-3">
                            <LoadCalculator
                              ...
                              onChange={(broadsides, repairSets) => updateRoleLoadout(setupIndex, roleIndex, broadsides, repairSets)}
                              className="bg-sail-white/20 rounded p-3"
                            />
                          </div>
                        )}
