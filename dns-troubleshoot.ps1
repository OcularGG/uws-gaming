...
Write-Host "❌ DNS is pointing to 8.8.8.8 (Google's public DNS server)" -ForegroundColor Red
Write-Host "❌ This is INCORRECT for Google Cloud Run domain mapping" -ForegroundColor Red
Write-Host ""
...
try {
    ...
    Write-Host "✓ Direct URL Status: $($response.StatusCode) - Cloud Run service is working!" -ForegroundColor Green
} catch {
    Write-Host "✗ Direct URL Status: Error - $($_.Exception.Message)" -ForegroundColor Red
}
...
Write-Host "❌ Problem: DNS points to 8.8.8.8 (wrong)" -ForegroundColor Red
Write-Host "✓ Solution: Change A records to 216.239.32.21, 216.239.34.21, 216.239.36.21, 216.239.38.21" -ForegroundColor Green
Write-Host "✓ Cloud Run service is working properly" -ForegroundColor Green
Write-Host ""
